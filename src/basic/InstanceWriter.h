...
public:
	...
	int write(const Instance *pInstance)
	{
	  ...
	  for (int i = 0; i < pInstance->charsize(); ++i) {
		  ...
		  m_outf << pInstance->chars[i] << " " << "-NULL-" << " " << pInstance->result.charlabels[i] << " " << pInstance->result.charheads[i];
  ...
  int write(const CResult& result)
  {
	...
	for (int i = 0; i < result.charsize(); ++i) {
		...
		m_outf << (*result.chars)[i] << " " << "-NULL-" << " " << result.charlabels[i] << " " << result.charheads[i];

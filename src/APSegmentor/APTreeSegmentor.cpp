...
int TreeSegmentor::createAlphabet(const vector<Instance>& vecInsts) {
	...
	CResult output;
	CAction answer;
	Metric evalarc, evalseg;
	int actionNum;
	evalarc.reset(); evalseg.reset();
	for (int numInstance = 0; numInstance < totalInstance; numInstance++) {
		...
		instance.evaluate(output, evalseg, evalarc);

		if (!evalseg.bIdentical() || !evalarc.bIdentical()) {
...
void TreeSegmentor::getGoldActions(const vector<Instance>& vecInsts, vector<vector<CAction> >& vecActions){
	vecActions.clear();
...
void TreeSegmentor::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
	if (optionFile != "")
	...
	m_driver._hyperparams.action_num = CAction::SEP + 1;
	...
	static Metric eval;
	static Metric metricseg_dev, metricarc_dev;
	static Metric metricseg_test, metricarc_test;
	...
	for (int iter = 0; iter < maxIter; ++iter) {
		...
		if (bEvaluate && devNum > 0) {
			...
			metricseg_dev.reset(); metricarc_dev.reset(); 
			for (int idx = 0; idx < devInsts.size(); idx++) {
				predict(devInsts[idx], curDecodeInst);
				devInsts[idx].evaluate(curDecodeInst, metricseg_dev, metricarc_dev);
				if (!m_options.outBest.empty()) {
			...
			if (!m_options.outBest.empty() && metricseg_dev.getAccuracy() > bestFmeasure) {
			...
			if (testNum > 0) {
				...
				for (int idx = 0; idx < testInsts.size(); idx++) {
					...
					testInsts[idx].evaluate(curDecodeInst, metricseg_test, metricarc_test);
			...
			for (int idx = 0; idx < otherInsts.size(); idx++) {
				...
				for (int idy = 0; idy < otherInsts[idx].size(); idy++) {
					...
					otherInsts[idx][idy].evaluate(curDecodeInst, metricseg_test, metricarc_test);
					if (bCurIterBetter && !m_options.outBest.empty()) {
						decodeInstResults.push_back(curDecodeInst);
					}
				}
			...
			if (m_options.saveIntermediate && metricseg_dev.getAccuracy() > bestFmeasure) {
				std::cout << "Exceeds best previous DIS of " << bestFmeasure << ". Saving model file.." << std::endl;
				bestFmeasure = metricseg_dev.getAccuracy();
...
void TreeSegmentor::predict(const Instance& input, CResult& output) {
	m_driver.decode(input.chars, output);
}

void TreeSegmentor::test(const string& testFile, const string& outputFile, const string& modelFile) {
	loadModelFile(modelFile);
	vector<Instance> testInsts;
	m_pipe.readInstances(testFile, testInsts, m_options.maxInstance);

	vector<CResult > testInstResults(testInsts.size());
	Metric metricseg_test, metricarc_test;
	metricseg_test.reset(); metricarc_test.reset();
	for (int idx = 0; idx < testInsts.size(); idx++) {
		vector<string> result_labels;
		predict(testInsts[idx], testInstResults[idx]);
		testInsts[idx].evaluate(testInstResults[idx], metricseg_test, metricarc_test);
	}
	std::cout << "test:" << std::endl;
	metricseg_test.print();
	metricarc_test.print();
	...
	for (int idx = 0; idx < testInsts.size(); idx++) {
		...
		for (int idy = 0; idy < testInstResults[idx].charsize(); idy++){
			...
			os << (*testInstResults[idx].chars)[idy] << " " << "-NULL-" << " " << testInstResults[idx].charlabels[idy]
...
void TreeSegmentor::loadModelFile(const string& inputModelFile) {

}

void TreeSegmentor::writeModelFile(const string& outputModelFile) {
